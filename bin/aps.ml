...
| Call_indirect of var * var list * frame
...
let rec pp_expr (subs: string VarMap.t) (fmt: Format.formatter) (cps : expr) : unit =
  ...
  | Call_indirect (x, args, (k, kargs)) -> Format.fprintf fmt "\tk%d (%s %a) %a" k (gen_name x subs) (pp_args ~split:" " ~subs ~empty: "()") args (pp_args ~split:" " ~subs ~empty: "()") kargs
...
let rec clean_expr (expr: expr) (alias: var VarMap.t): expr =
  ...
  | Call_indirect (x, args, (k, kargs)) -> Call_indirect (update_var x alias, update_vars args alias, (k, update_vars kargs alias))
...
and expr_to_asm (cps: expr) (vars: var Seq.t): Asm.expr * int Seq.t =
  ...
  | Call_direct (k_id, clos, args, frame) -> begin
      let env_id, vars = inc vars in
      Let (env_id, Get (clos, 1), Apply_direct (k_id, env_id :: args, [frame])), vars
    end
  | Call_indirect (clos, args, frame) -> begin
      let k_id, vars = inc vars in
      let env_id, vars = inc vars in
