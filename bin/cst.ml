...
let rec to_cps (vars: Cps.var Seq.t) conts fv0 (ast : expr) var (expr : Cps.expr) (substitutions : (var * int) list) : Cps.expr * Cps.var Seq.t * (var * int) list * int list * Cps.cont =
  match ast with
  ...
  | Binary (binary_operator, e1, e2) -> begin
      ...
      let expr', vars, substitutions', fv', conts' = to_cps vars conts (e1_id :: fv0) e2 e2_id expr substitutions in
      let expr, vars, subs, fvs, conts = to_cps vars conts' (fv' @ fv0) e1 e1_id expr' (substitutions' @ substitutions) in
      expr, vars, subs @ substitutions', (fvs @ fv'), conts
    end
  (*
  ...
  *)
  | Fun (argument_name, body) -> begin
      ...
      let body_cps, vars, body_substitutions, body_free_variables, body_continuations = to_cps vars conts [] body body_return_id (Return body_return_id) [] in
      let body_argument_id, vars = if has_var_name body_substitutions argument_name then get_var_id body_substitutions argument_name, vars else inc vars in
      ...
      Let (var, Closure (closure_id, body_free_variables), expr), vars, body_substitutions, body_free_variables, (Let_clos (closure_id, body_free_variables, [body_argument_id], Apply_cont (function_id, body_argument_id :: body_free_variables), Let_cont (function_id, body_argument_id :: body_free_variables, body_cps, body_continuations)))
    end
  (*
  ...
  *)
  | Var variable_name -> begin
      ...
      then Let (var, Var (get_var_id substitutions variable_name), expr), vars, [], [], conts
        ...
        let variable_id, vars = inc vars in
        Let (var, Var variable_id, expr), vars, [(variable_name, variable_id)], [variable_id], conts
      end
    end
  | Let (var', e1, e2) -> begin
      let cps1, vars, substitutions1, fv1, conts1 = to_cps vars conts fv0 e2 var expr substitutions in
      let v1, vars = if has_var_name (substitutions1 @ substitutions) var' then get_var_id (substitutions1 @ substitutions) var', vars else inc vars in
      let cps2, vars, substitutions2, fv2, conts2 = to_cps vars conts1 (remove_var (fv1 @ fv0) v1) e1 v1 cps1 (List.filter (fun (_, v) -> not (v = v1)) (substitutions1 @ substitutions)) in
      cps2, vars, (if has_var_name (substitutions1 @ substitutions) var' then substitutions2 @ substitutions1 else add_var (substitutions2 @ substitutions1) var' v1), (fv2 @ (remove_var fv1 v1)), conts2
    end
    (*
        let env = fvs_1 ∪ ... ∪ fvs_n
  ...
  *)
  | Let_rec (bindings, scope) ->
    let scope_cps, vars, scope_substitutions, scope_free_variables, scope_conts = to_cps vars conts fv0 scope var expr substitutions in

    (*let scope_substitutions = scope_substitutions @ substitutions in*)
    
    (* Substitued binding variables in scope. *)
    let vars, scope_binding_variable_ids = List.fold_left_map (fun vars (var', _) -> if has_var_name scope_substitutions var' then vars, get_var_id scope_substitutions var' else begin
    ...
    let (vars, scope_and_closures_conts), closures = List.fold_left_map (fun (vars, scope_conts') (_, binding_expr) -> begin
      ...
      | Fun (arg, binding_body_expr) ->
          ...
          let binding_body_cps, vars, binding_body_substitutions, binding_body_free_variables, binding_body_conts = to_cps vars scope_conts' [] binding_body_expr return_variable (Return return_variable) [] in
          (vars, binding_body_conts), (arg, binding_body_cps, binding_body_substitutions, binding_body_free_variables)
      | _ -> assert false
    end) (vars, scope_conts) bindings in

    let vars, closures2 = List.fold_left_map (fun vars (scope_binding_variable_id, (arg, binding_body_cps, binding_body_substitutions, binding_body_free_variables)) ->
      (* Substitued binding variables in body. *)
      let vars, bindind_body_bindind_variable_ids = List.fold_left_map (fun vars (binding_name, _) -> if has_var_name binding_body_substitutions binding_name then vars, get_var_id binding_body_substitutions binding_name else begin
        let var_id, vars = inc vars in
        vars, var_id
      end) vars bindings in
      
      (* Substitued arg in body. *)
      let binding_body_arg_id, vars = if has_var_name binding_body_substitutions arg then get_var_id binding_body_substitutions arg, vars else inc vars in
      ...
      vars, (scope_binding_variable_id, inc_conts (), binding_body_cps, binding_body_substitutions, bindind_body_bindind_variable_ids, binding_body_arg_id, binding_body_free_variables_no_arg_no_bindings)) vars (List.combine scope_binding_variable_ids closures) in
    ...
    let vars, all_binding_bodies_substitutions = List.fold_left (fun (vars, substitutions') fv -> if has_var_name substitutions fv then vars, add_var substitutions' fv (get_var_id substitutions fv) else if has_var_name substitutions' fv then vars, substitutions' else begin
      let var_id, vars = inc vars in
      vars, add_var substitutions' fv var_id
    end) (vars, []) (List.concat closures_free_variable_names) in
    ...
    let vars, scope_cps, _scope_free_variables_no_bindings'', scope_and_closures_conts = List.fold_left (fun (vars, scope_cps', _scope_free_variables_no_bindings', scope_and_closures_conts') ((scope_binding_variable_id, binding_body_closure_continuation_id, binding_body_cps, _binding_body_substitutions, bindind_body_bindind_variable_ids, binding_body_arg_id, binding_body_free_variables_no_arg_no_bindings), caller_free_variable_ids) ->
      (* *)
    ...
    scope_cps, vars, all_binding_bodies_substitutions @ scope_substitutions_no_bindings, join_fv all_binding_bodies_free_variables scope_free_variables_no_bindings, scope_and_closures_conts
    (*
  ...
  | If (condition_expr, true_expr, false_expr) -> begin
      ...
      let true_cps, vars, true_substitutions, true_free_variables_id, true_continuations = to_cps vars (Let_cont (merge_continuation_id, var :: fv0, expr, conts)) fv0 true_expr true_id (Apply_cont (merge_continuation_id, true_id :: fv0)) [] in
      let false_cps, vars, false_substitutions, false_free_variables_id, true_and_false_continuations = to_cps vars true_continuations fv0 false_expr false_id (Apply_cont (merge_continuation_id, false_id :: fv0)) [] in
      ...
      let vars, true_false_substitutions = List.fold_left (fun (vars, substitutions') fv -> if has_var_name (substitutions' @ substitutions) fv then vars, substitutions' else begin
        let var_id, vars = inc vars in
        vars, add_var substitutions' fv var_id
      end) (vars, []) (true_expr_free_variables_names @ false_expr_free_variables_names) in
  ...
  | App (closure_expr, argument_expr) -> begin
      ...
      let cps1, vars, substitutions1, fv1, conts1 = to_cps vars (Let_cont (return_continuation, var :: fv0, expr, conts)) (closure_id :: fv0) argument_expr argument_id (Call (closure_id, [argument_id], (return_continuation, fv0))) substitutions in
      let cps2, vars, substitutions2, fv2, conts2 = to_cps vars conts1 (fv1 @ fv0) closure_expr closure_id cps1 (substitutions1 @ substitutions) in
      cps2, vars, substitutions2 @ substitutions1, fv2 @ fv1, conts2
    end
  (*
  ...
  | Match (pattern_expr, branchs, default_branch_expr) -> begin
      ...
      let default_branch_cps, vars, default_branch_substitutions, default_branch_free_variables, default_branch_continuations = to_cps vars conts fv0 default_branch_expr default_branch_return_id (Apply_cont (return_continuation_id, default_branch_return_id :: fv0)) [] in
      ...
      let (vars, branchs_continuations), branchs_bodies = List.fold_left_map (fun (vars, default_continuation') (branch_index, branch_arguments_names, branch_expr) -> begin
        ...
        let branch_cps, vars, branch_substitutions, branch_free_variables, branch_continuations = to_cps vars default_continuation' fv0 branch_expr branch_return_id (Apply_cont (return_continuation_id, branch_return_id::fv0)) [] in
        
        (* Branch arguments substitutions. *)
        let vars, branch_arguments_ids = List.fold_left_map (fun vars branch_argument_name -> if has_var_name branch_substitutions branch_argument_name then vars, get_var_id branch_substitutions branch_argument_name else begin
          let var_id, vars = inc vars in
          vars, var_id
        end) vars branch_arguments_names in
        ...
        (vars, Let_clos (branch_continuation_id, branch_arguments_ids, branch_free_variables @ fv0, branch_cps, branch_continuations)), (branch_index, branch_continuation_id, branch_free_variables, branch_substitutions)
      ...
      (* Substitution of free variables. *)
      let vars, substitutions_branchs = List.fold_left (fun (vars, substitutions') fv -> if has_var_name (substitutions' @ substitutions) fv then vars, substitutions' else begin
        let var_id, vars = inc vars in
        vars, add_var substitutions' fv var_id
      end) (vars, []) (List.concat free_variable_names_branchs @ free_variable_names_default) in

      (* Substitued free variables. *)
      let free_variables_branchs = List.map (fun (_, _, fv2, substitutions_e2) -> List.map (fun fv -> if has_var_id substitutions_e2 fv then let fval = get_var_name substitutions_e2 fv in if has_var_name (substitutions_branchs @ substitutions) fval then get_var_id (substitutions_branchs @ substitutions) fval else fv else fv) fv2) branchs_bodies in
      let free_variables_default = List.map (fun fv -> if has_var_id default_branch_substitutions fv then let fval = get_var_name default_branch_substitutions fv in if has_var_name (substitutions_branchs @ substitutions) fval then get_var_id (substitutions_branchs @ substitutions) fval else fv else fv) default_branch_free_variables in

      (* Pattern matching. *)
      let pattern_id, vars = inc vars in
      let expr, vars, subs, fvs, conts = to_cps vars branchs_continuations (join_fv free_variables_default (join_fvs free_variables_branchs)) pattern_expr pattern_id (Match_pattern (pattern_id, List.map (fun ((n, k, _, _), fvs) -> n, k, fvs @ fv0) (List.combine branchs_bodies free_variables_branchs), (default_continuation_id, free_variables_default @ fv0))) (substitutions_branchs @ substitutions) in

      expr, vars, subs @ substitutions_branchs, fvs @ (join_fv free_variables_default (join_fvs free_variables_branchs)), conts
    end
    (*
  ...
  | Tuple args -> begin
     ...
     List.fold_left (fun (expr', vars, substitutions', fv', conts') (var, e) -> let expr'', vars, substitutions'', fv'', conts'' = to_cps vars conts' ((remove_var fv' var) @ fv0) e var expr' (substitutions' @ substitutions) in expr'', vars, (substitutions'' @ substitutions'), fv'' @ (remove_var fv' var), conts'') (Let (var, Tuple (List.map (fun (var, _) -> var) args_ids), expr), vars, [], (List.map (fun (var, _) -> var) args_ids), conts) args_ids
    end
    (*
  ...
  | Constructor (tag, args) -> begin
    let vars, args_ids = List.fold_left_map (fun vars arg -> begin
     ...
     List.fold_left (fun (expr', vars, substitutions', fv', conts') (var, e) -> let expr'', vars, substitutions'', fv'', conts'' = to_cps vars conts' ((remove_var fv' var) @ fv0) e var expr' (substitutions' @ substitutions) in expr'', vars, (substitutions'' @ substitutions'), fv'' @ (remove_var fv' var), conts'') (Let (var, Constructor (tag, List.map (fun (var, _) -> var) args_ids), expr), vars, [], (List.map (fun (var, _) -> var) args_ids), conts) args_ids
    end
