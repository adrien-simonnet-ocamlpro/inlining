...
let rec to_cps (vars: Cps.var Seq.t) (pointers: Cps.pointer Seq.t) fv0 (ast : expr) var (expr : Cps.expr): Cps.expr * Cps.var Seq.t * Cps.pointer Seq.t * int list * Cps.blocks =
  ...
  *)
  ...
  (*
      ...
      expr
  *)
  | Var variable_name -> Let (var, Var variable_name, expr), vars, pointers, [variable_name], empty_blocks
  | Let (var', e1, e2) -> begin
      let cps1, vars, pointers, fv1, conts1 = to_cps vars pointers fv0 e2 var expr in
      let cps2, vars, pointers, fv2, conts2 = to_cps vars pointers (remove_var (join_fv fv1 fv0) var') e1 var' cps1 in
      cps2, vars, pointers, join_fv fv2 (remove_var fv1 var'), join_blocks conts1 conts2
    end
    (*
        let env = fvs_1 ∪ ... ∪ fvs_n
  ...
  *)
  | Let_rec (bindings, scope) ->
    ...
    let (vars, pointers, scope_and_closures_conts), closures = List.fold_left_map (fun (vars, pointers, scope_conts') (_, binding_expr) -> begin
      ...
      | Fun (arg, binding_body_expr) ->
          ...
          let binding_body_cps, vars, pointers, binding_body_free_variables, binding_body_conts = to_cps vars pointers [] binding_body_expr return_variable (Return return_variable) in
    ...
    let vars, pointers, scope_cps, _scope_free_variables_no_bindings'', scope_and_closures_conts = List.fold_left (fun (vars, pointers, scope_cps', _scope_free_variables_no_bindings', scope_and_closures_conts') ((scope_binding_variable_id, binding_body_closure_continuation_id, binding_body_cps, bindind_body_bindind_variable_ids, binding_body_arg_id, binding_body_free_variables_no_arg_no_bindings), caller_free_variable_ids) ->
      (* *)
  ...
  | App (e1, e2) -> begin
      ...
      let cps1, vars, pointers, fv1, conts1 = to_cps vars pointers (e1_id :: fv0) e2 e2_id (Call (e1_id, [e2_id], (return_kid, fv0))) in
      let cps2, vars, pointers, fv2, conts2 = to_cps vars pointers (join_fv fv1 fv0) e1 e1_id cps1 in
      cps2, vars, pointers, join_fv fv2 fv1, add_block return_kid (Return (var, fv0), expr) (join_blocks conts2 conts1)
    end
  (*
  ...
  | Match (pattern_expr, branchs, default_branch_expr) -> begin
      ...
      let default_branch_cps, vars, pointers, default_branch_free_variables, default_branch_continuations = to_cps vars pointers fv0 default_branch_expr default_branch_return_id (Match_return (return_continuation_id, default_branch_return_id, fv0)) in
      ...
      let (vars, pointers, branchs_continuations), branchs_bodies = List.fold_left_map (fun (vars, pointers, default_continuation') (branch_index, branch_arguments_names, branch_expr) -> begin
        ...
        let branch_cps, vars, pointers, branch_free_variables, branch_continuations = to_cps vars pointers fv0 branch_expr branch_return_id (Match_return (return_continuation_id, branch_return_id, fv0)) in
        ...
        (vars, pointers, add_block branch_continuation_id (Match_branch (branch_arguments_names, branch_free_variables, fv0), branch_cps) (join_blocks branch_continuations default_continuation')), (branch_index, branch_continuation_id, branch_free_variables)
  ...
  | Tuple args -> begin
     ...
     List.fold_left (fun (expr', vars, pointers, fv', conts') (var, e) -> let expr'', vars, pointers, fv'', conts'' = to_cps vars pointers ((remove_var fv' var) @ fv0) e var expr' in expr'', vars, pointers, join_fv fv'' (remove_var fv' var), join_blocks conts' conts'') (Let (var, Tuple (List.map (fun (var, _) -> var) args_ids), expr), vars, pointers, (List.map (fun (var, _) -> var) args_ids), empty_blocks) args_ids
    end
    (*
  ...
  | Constructor (tag, args) -> begin
    let vars, args_ids = List.fold_left_map (fun vars arg -> begin
     ...
     List.fold_left (fun (expr', vars, pointers, fv', conts') (var, e) -> let expr'', vars, pointers, fv'', conts'' = to_cps vars pointers ((remove_var fv' var) @ fv0) e var expr' in expr'', vars, pointers, join_fv fv'' (remove_var fv' var), join_blocks conts' conts'') (Let (var, Constructor (tag, List.map (fun (var, _) -> var) args_ids), expr), vars, pointers, (List.map (fun (var, _) -> var) args_ids), empty_blocks) args_ids
    end
